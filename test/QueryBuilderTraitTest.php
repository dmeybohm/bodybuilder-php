...
class QueryBuilderTraitTest extends BaseTestCase
{
    ...
    {
        $this->plan(1);
        $result = queryBuilder()->query(QueryType::MatchPhrase, 'message', 'this is a test');
    ...
    {
        $this->plan(1);
        $result = queryBuilder()->query(QueryType::MatchPhrase, 'message', array("query" => 'this is a test', "analyzer" => 'my_analyzer'));
    ...
    {
        $this->plan(1);
        $result = queryBuilder()->query(QueryType::QueryString, 'query', 'this AND that OR thus');
    ...
    {
        $this->plan(1);
        $result = queryBuilder()->query(QueryType::QueryString, 'query', 'this AND that OR thus', array("fields" => array('content', 'name')));
    ...
    {
        $this->plan(1);
        $result = queryBuilder()->query(QueryType::QueryString, array("query" => 'this AND that OR thus', "fields" => array('content', 'name')));
        $this->assertEquals($result->getQuery(), array("query_string" => array("query" => 'this AND that OR thus', "fields" => array('content', 'name'))));
    }
    public function testQueryBuilderSimpleQueryString()
    {
        $this->plan(1);
        $result = queryBuilder()->query(QueryType::SimpleQueryString, 'query', 'foo bar baz');
    ...
    {
        $this->plan(1);
        $result = queryBuilder()->query(QueryType::ConstantScore, array("boost" => 1.2), function (FilterAndQueryBuilder $q) {
    ...
    {
        $this->plan(1);
        $result = queryBuilder()->query(QueryType::HasChild, 'type', 'blog_tag', function (FilterAndQueryBuilder $q) {
    ...
    {
        $this->plan(1);
        $result = queryBuilder()->query(QueryType::HasParent, 'parent_tag', 'blog', function (FilterAndQueryBuilder $q) {
            return $q->query(QueryType::Term, 'tag', 'something');
        });
    ...
    {
        $this->plan(1);
        $result = queryBuilder()->query(QueryType::GeoBoundingBox, 'pin.location', array("top_left" => array("lat" => 40, "lon" => -74), "bottom_right" => array("lat" => 40, "lon" => -74)), array("relation" => 'within'));
    ...
    {
        $this->plan(1);
        $result = queryBuilder()->query(QueryType::GeoDistance, 'pin.location', array("lat" => 40, "lon" => -74), array("distance" => '200km'));
    ...
    {
        $this->plan(1);
        $result = queryBuilder()->query(QueryType::GeoDistanceRange, 'pin.location', array("lat" => 40, "lon" => -74), array("from" => '100km', "to" => '200km'));
    ...
    {
        $this->plan(1);
        $result = queryBuilder()->query(QueryType::GeoPolygon, 'person.location', array("points" => array(array("lat" => 40, "lon" => -70), array("lat" => 30, "lon" => -80), array("lat" => 20, "lon" => -90))));
    ...
    {
        $this->plan(1);
        $result = queryBuilder()->query(QueryType::GeohashCell, 'pin', array("lat" => 13.408, "lon" => 52.5186), array("precision" => 3, "neighbors" => true));
    ...
    {
        $this->plan(1);
        $result = queryBuilder()->query(QueryType::MoreLikeThis, array("fields" => array('title', 'description'), "like" => "Once upon a time", "min_term_freq" => 1, "max_query_terms" => 12));
    ...
    {
        $this->plan(1);
        $result = queryBuilder()->query(QueryType::Or_, array(array("term" => array("user" => 'kimchy')), array("term" => array("user" => 'tony'))));
